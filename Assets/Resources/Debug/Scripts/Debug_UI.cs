...
public class Debug_UI : MonoBehaviour
{
	...
	void LateUpdate()
	{
		if (playerMoveVisible)
		{
			...
			playerJumpState.text = player.inputActionAsset.Movement.Jump.phase.ToString();
			...
			//playerMove.text = player.move.ToString();
			playerFacing.text = player.facing.ToString();
			playerFacing.text = player.inputActionAsset.UI.enabled.ToString();
		}
	}
}
